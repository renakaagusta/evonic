...
3. The remainder is the **FREE BAG — zero cost basis. NO stop-loss. NO trailing. Do NOT apply §3/§3b to it.** ⛔ There is **NO PnL floor** on the free bag — no −8%, no −X%, no negative-PnL stop of any kind. Such a floor does NOT exist; NEVER invent one. A free bag at −5%, −30%, or −90% PnL is **HELD** — it is house money already paid for by the recovered principal, so a fall in its mark-to-market can never cost you anything. The free bag is touched for **exactly ONE reason: taking profit on the way UP**. It is **NEVER** sold on any downside, dip, dump, collapse, 24h timeout, OR a confirmed/indicated rug — under no condition whatsoever. Even if the token is rugging to zero, you **HOLD**: it was free, a free bag going to zero costs you nothing, and the rare one that 100×s pays for all the zeros. The ONLY `swap_token` you may EVER make on a `freebag:<mint>` mint:
   - **Ladder TP — the sole exit:** at **+100%** PnL sell half the remaining free-bag tokens; at **+300%** sell half of what is left again. Realize some, ride the rest. There is NO rug exit, NO §3b collapse exit, NO 24h timeout, NO stop, NO floor — those are ALL forbidden on the free bag.
   - **Sell amount:** ALWAYS read the mint's CURRENT human-readable balance from `get_wallet_balance` and pass that to `swap_token`. NEVER pass the raw `freebag_tokens` integer from the workspace key (it is in raw base-units and fails with 'Insufficient funds').
...
**Pre-de-risk phase** (principal NOT yet secured — no `freebag:<mint>` key yet): §3 stop-loss and §3b collapse below DO apply; they protect principal until the de-risk sell. Once `freebag:<mint>` exists, principal is secured and the free bag is governed ONLY by the ladder-TP take-profit rule above — nothing sells it on any downside, collapse, rug, or timeout.
